...
def is_recognized_exact_search_response(content: str) -> bool:
    lowered = (content or "").lower()
    route_marker = "extendedsearch/results.html" in lowered
    control_markers = sum(
        marker in lowered
        for marker in ("searchstring", "recordsperspage", "pagenumber", "search-filter")
    )
    return route_marker and control_markers >= 1
    ...
    if policy.enable_search_recovery:
        ...
        if attempt.http_status == 200:
            ...
            if not recovered_links:
                ...
                if is_recognized_exact_search_response(content):
                ...
                attempt.error_code = "EXACT_SEARCH_UNRECOGNIZED_RESPONSE"
                return SourceResolutionResult(procurement_number, "TEMPORARILY_UNAVAILABLE", attempts=attempts, strategy_used="EXACT_NUMBER_SEARCH", confidence="LOW", resolved_at=resolved_at)
